...
function SidebarMenu({
  ...
  return (
    <>
      {isOpen && <div onClick={onClose} className="fixed inset-0 bg-black/50 z-50 backdrop-blur-xs" />}
      <aside 
        key={`sidebar-${forceUpdate}`}
        className={`fixed top-0 bottom-0 left-0 w-[290px] bg-[#113824] text-white z-50 flex flex-col justify-between p-5 transition-transform duration-300 ${isOpen ? 'translate-x-0' : '-translate-x-full'}`}
      >
        <div className="space-y-6">
          <button onClick={onGoHome} className="flex items-center gap-3 pb-2 text-left transition-transform active:scale-95 cursor-pointer">
        ...
        <div className="space-y-4 pt-4 border-t border-emerald-800/60">
          ...
          <div className="bg-[#184a30] rounded-xl p-3 flex items-center gap-3">
            <div className="w-9 h-9 rounded-full bg-emerald-700 text-emerald-200 flex items-center justify-center font-bold text-sm shrink-0">
              {displayName[0]?.toUpperCase()}
            </div>
            <div className="min-w-0 flex-1">
              <p className="text-xs font-bold text-white truncate">{displayName}</p>
              <p className="text-[10px] text-emerald-300/60 truncate">{userEmail}</p>
            </div>
          </div>
          <button onClick={onSignOut} className="w-full flex items-center gap-2 px-3 py-2 text-xs font-bold text-emerald-300/80">
            Cerrar sesión
          </button>
        </div>
      </aside>
...
export default function Layout({ children }: { children: React.ReactNode }) {
  ...
  const { isPremium, forceUpdate } = usePremium()
    ...
    <div className="min-h-screen w-full bg-white text-slate-800 font-sans antialiased relative">
      <header className="bg-white border-b border-slate-100/80 px-4 py-3 flex items-center justify-between sticky top-0 z-40">
        <div className="flex items-center gap-3">
          ...
          <button onClick={() => navigate('/')} className="transition-transform active:scale-95 cursor-pointer">
            <img src="/logo.png" alt="Secret Garden" className="w-9 h-9 object-contain" />
          </button>
        </div>
        <button
      ...
      <div className="fixed bottom-5 right-5 z-50">
        {showChat ? (
          <button onClick={() => setShowChat(false)} className="w-14 h-14 bg-[#009660] text-white rounded-full shadow-lg flex items-center justify-center text-xl transition-all transform active:scale-90">
            ✕
          </button>
        ) : (
          <button onClick={() => setShowChat(true)} className="w-14 h-14 bg-white rounded-full shadow-md border border-slate-100 flex items-center justify-center transition-all transform active:scale-90 relative">
